...
const Products = () => {
    ...
    const [sanitaryProductReference, setSanitaryProductReference] =
    useState();
    ...
    const getProducts = () => {
        api
            ...
            .then((response)=>{
                setProductsData(
                    arraySort.alphabetical(response, "productReference")
                )
            })
    ...
    useEffect(()=>{
        ...
        for (const equivalence of equivalences) { 
            componentRelations(true, equivalence._id, equivalence.productReference, 'product');
        };
        ...
        for (const component of components){
            componentRelations(true, component._id, component.componentReference, 'component');
        }
    ...
    const newProduct = () => {
        const newProduct = {
            ...
            components: TreatyArray.dropDuplicatesAndEmpty(componentsList.map((item)=>(item.id))),
        ...
        {isNew ? 
            api
            .post('/products/create', newProduct)
            .then((response)=>{
                getProducts()
            })
    ...
    switch (item) {
        ...
        const filteredProducts = productsData.filter(product =>
            product.productReference.toLowerCase().includes(value.toLowerCase()) || product.description.toLowerCase().includes(value.toLowerCase())
        );
        ...
        {componentDetailVisible && (
            ...
            {isEdit || isNew ? (
                ...
                <div className="headbord-filter">
                    ...
                    <input placeholder="buscar ref. o nom" onChange={(e)=>fnFilter('product', e.target.value)}/>
                </div>
                ...
                <div className="optionsSelector">
                    ...
                    <div className="list-container">
                        {componentsDataFiltered &&
                            ...
                            <input
                                type="checkbox"
                                className="checkbox-style"
                                checked = {componentsList.some((item)=>item.id === component._id)}
                                onChange={(e) => {
                                componentRelations(
                                    e.target.checked,
                                    component._id,
                    ...
                    {equivalencesDataFiltered &&
                        ...
                        <input
                            type="checkbox"
                            className="checkbox-style"
                            checked = {equivalencesList.some((item)=>item.id === product._id)}
                            onChange={(e) => {
                            componentRelations(
                                e.target.checked,
                                product._id,
                ...
                <h1>{productSelected.productReference}</h1>
    
                <p>
                    Referència sanitària:{" "}
                    {productSelected.sanitaryProductReference}
                </p>
                <p>Nom: {productSelected.description}</p>
                <p>Preu: {productSelected.price} €</p>
                <h2>Components</h2>
